<div id="comments">
  ...
  <?php if( $comments ) : ?>
    ...
    <ol>
      <?php foreach( $comments as $comment ) :
              if( get_comment_type()=='comment' ){ ?> <!-- checks for comment type and outputs only comments -->
      <li id="comment-<?php comment_ID(); ?>" class="comment <?php echo ( $i&1 ? 'odd' : 'even' ); ?> <?php $user_info = get_userdata( 1 ); if( $user_info->ID==$comment->user_id ) echo 'authorComment'; ?> <?php if( $comment->user_id>0 ) echo 'user-comment'; ?>">
      <?php if( $comment->comment_approved=='0' ){ ?> <!-- if comment is awaiting approval -->
        <p class="waiting-for-approval"><em>Your comment is awaiting approval.</em></p><?php } ?>
        <div class="comment-text"><?php comment_text(); ?></div><!--.commentText-->
        <div class="comment-meta">
          <?php edit_comment_link( 'Edit Comment' , '' , '' ); ?>
          <?php comment_type(); ?> by <?php comment_author_link(); ?> on <?php comment_date(); ?> at <?php comment_time(); ?>
          <?php if( function_exists( 'get_avatar' ) ){ ?>
          <p class="gravatar"><?php echo get_avatar( $comment , '36' ); ?></p><?php } ?>
        </div><!--.commentMeta-->
      </li>
    <?php } else {
            $trackback = true;
  ...
  <?php
    ...
    <p><small>By submitting a comment you grant <?php bloginfo('name'); ?> a perpetual license to reproduce your words
      and name/web site in attribution. Inappropriate and irrelevant comments will be removed at an admin’s discretion.
      Your email is used for verification purposes only, it will never be shared.</small></p>
  <?php
      }
    }else{ ?>
